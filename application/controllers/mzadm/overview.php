...
class Overview extends CI_Controller {
	 ...
	 * 		http://aeti.or.id/mzadm/overview
	 *	- or -  
	 * 		http://aeti.or.id/mzadm/overview/index
	 *	- or -
	...
	public function index()
	{
		...
		$h_data = array(
					'page_id'		=> '',
					'page_title'	=> 'AETI - Organization Overview',
					'page_css'		=> ''
				);
		...
		if (!empty($this->uri->rsegment(3)) && is_numeric($this->uri->rsegment(3))) {
			...
			if(!empty($post_data)){
				...
				if ($this->form_validation->run() == TRUE)
				{
					...
					$this->load->model('article');
					$this->article->update_overview($post_data);
					$ra_data['msg_success'] = "Edit Organization Overview Success";
					$this->session->set_flashdata($ra_data);
					redirect(site_url().'mzadm/overview', 'location');
				}else{
			...
			$this->load->model('article');
			$overview_data = $this->article->get_overview($this->uri->rsegment(3));
			// var_dump($overview_data);exit;
			$h_data = array(
						'page_id'		=> '',
...
/* Location: ./application/controllers/mzadm/overview.php */